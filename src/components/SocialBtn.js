...
const SocialBtn = ({ title, icon, signUp }) => {
    ...
    const clickHandler = async () => {
        ...
        if (title.trim() == 'Google') {
            ...
            GoogleSignin.configure({
                scopes: ['email'],
                webClientId: '412907293114-1utcic8dehsmsr0kadritmeoajpmar48.apps.googleusercontent.com',
                offlineAccess: true
    ...
    const emailAlreadyUseAlert = () => {
        return (Alert.alert('Error', 'Your email address is already configured. Please Login with the email option.', [
            {
                text: 'Ok',
    ...
    if (loading) {
        return (
            <View style={{ alignItems: 'center', justifyContent: 'center', height: 60, backgroundColor: 'white' }}>
    ...
    return (
        ...
        >
            <View style={styles.icon} >
                <Image source={icon} style={{ width: 27, height: 30, resizeMode: 'contain' }} />
            </View>
